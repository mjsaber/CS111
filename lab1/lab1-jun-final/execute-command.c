...
/*back up plan*/
cmd_list_t d_list;
cmd_list_t d_list_tail;

cmd_list_t p_list;
cmd_list_t p_list_tail;
...
{	
	cmd_list_t ptr = cmd_list_to_excute;
		...
		while(ptr!=NULL){
			if(ptr->node->parent_num == 0)//p_list
			{
				cmd_node_t temp =  checked_malloc(sizeof(struct cmd_node));
				temp->cmd = ptr->node->cmd;
				temp->child = ptr->node->child;
				temp->child_tail = NULL;
				temp->parent_num = 0;
				temp->files = ptr->node->files;
				...
				if(p_list == NULL) //if the whole command link is null
				{
					p_list = new_node;
					p_list_tail = p_list;
				}
				...
				{
					p_list_tail->next = new_node;
					p_list_tail = p_list_tail->next;
				}
			}
			else{
				cmd_node_t temp =  checked_malloc(sizeof(struct cmd_node));
				...
				temp->child = ptr->node->child;
				temp->child_tail = NULL;
				temp->parent_num = ptr->node->parent_num;
				temp->files = ptr->node->files;
				...
				if(d_list == NULL) //if the whole command link is null
				{
					d_list = new_node;
					d_list_tail = d_list;
				}
				...
				{
					d_list_tail->next = new_node;
					d_list_tail = d_list_tail->next;
				}
...
void execute_parallel()
{
		...
		cmd_list_t ptr2 = d_list;
		while(ptr2 != NULL)
		{
			exec_cmd(ptr2->node->cmd);
			ptr2 = ptr2->next;
		}
