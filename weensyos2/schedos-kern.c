...
start(void)
{
	...
	for (i = 1; i < NPROCS; i++) {
		...
		proc_array[i].p_proportion = i;
		//set pro1 counter to 1, others to 0
		if (i ==1)
			proc_array[i].p_counter = 1;
		else 
...
schedule(void)
{
	...
	else if (scheduling_algorithm == 2)
	{
		...
		while (1)
		{
			pid = (pid+1) % NPROCS;//keep moving
			highest_priority = NPROCS+1;//won't reach 
			int i;
			for (i=0;i<NPROCS;i++)
			{
				if (proc_array[i].p_state == P_RUNNABLE && proc_array[i].p_priority <=highest_priority)
	...
	while(1)
	{
		if(proc_array[pid].p_state == P_RUNNABLE)
		{
			if (proc_array[pid].p_proportion >
				proc_array[pid].p_counter)
			{
				proc_array[pid].p_counter++;
				run(&proc_array[pid]);
			}
			else
				proc_array[pid].p_counter=0;
