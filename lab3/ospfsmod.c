...
{
	/* EXERCISE: Your code here */
		//suppose we only have one bitmap block*/
	uint32_t total_blocks =  ospfs_super->os_nblocks;
	uint32_t i;
	void* start_block = ospfs_block(OSPFS_FREEMAP_BLK);//start from block 2 in bitmap
	for(i = 0; i < total_blocks; i++)
	{
		if(bitvector_test(start_block,i))
		{
			bitvector_clear(start_block,i);
			return i;
		}
	}
	return 0;
...
free_block(uint32_t blockno)
{
	/* EXERCISE: Your code here */
		//3 + 128/16
	uint32_t lower_bound = ospfs_super->os_firstinob + ospfs_super->os_ninodes/OSPFS_BLKINODES; 
...
add_block(ospfs_inode_t *oi)
{
	...
	if(indir2_index(n) == 0) // indirect2 part
	{
		//eprintk("indir2 called\n");
		uint32_t i = direct_index(n);
		uint32_t i2 = indir_index(n);
		uint32_t in2block = 0;

		if(oi->oi_indirect2 == 0) // indirect2 is empty
		{
			in2block = allocate_block();
			if(in2block == 0)
				return -ENOSPC;
		...
		uint32_t *ptr = NULL;//pointer to indirect inside indirect2
		uint32_t *ptr2 = NULL;//pointer to indirect2 block
		if( i == 0) //indirect in doubly indrirect but empty
		{
			...
			if(allocated[0] != 0)
				allocated[1] = ospfs_block(in2block2); //only happen when first if statement happened
			else
...
remove_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	if(indir_index(n) == 0) //in indirect part
	{
		uint32_t i = direct_index(n);
		uint32_t *temp_block = ospfs_block(oi->oi_indirect);
		//free specific pos
		free_block(temp_block[i]);
		temp_block[i] = 0;
		//if i = 0, have to free oi_indirect too, so no more
		if(i == 0)
		{
	...
	else if(indir2_index(n) == 0) // indirect2 part
	{
		uint32_t i = direct_index(n);
		uint32_t i2 = indir_index(n);
		uint32_t *temp1 = ospfs_block(oi->oi_indirect2);
		uint32_t *temp2 = ospfs_block(temp1[i2]);
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		//read from f_pos to end
		n=OSPFS_BLKSIZE - (*f_pos % OSPFS_BLKSIZE);
		//in case no enough buffer
		if(n > count - amount)
            n = count - amount;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		uint32_t blockno = ospfs_inode_blockno(oi, *f_pos);
		//eprintk ("oisize %d, fp %d, blkno %llu\n", oi->oi_size, *f_pos, blockno);
		uint32_t n;
		char *data;

		if (blockno == 0) 
		{
			// eprintk ("bbbbbbbbbb %d\n", blockno);
			retval = -EIO;
			goto done;
		}
...
{
	//eprintk("create_blank_direntry called\n %d", dir_oi->oi_size);
	// Outline:
	...
	while(d_i < dir_oi->oi_size)
	{
		// eprintk("position %d\n", d_i);
		od = ospfs_inode_data(dir_oi,d_i);
		if(od->od_ino == 0)
			return od;
		d_i += OSPFS_DIRENTRY_SIZE;
	}

	//no empty entries
	int ab = add_block(dir_oi);
	if(ab < 0)
		return ERR_PTR(ab);

	ospfs_direntry_t *od2 = ospfs_inode_data(dir_oi,d_i);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	/* EXERCISE: Your code here. */
	// eprintk("hard_link called");
	ospfs_inode_t *d = ospfs_inode(dir->i_ino);
	if(find_direntry(d,dst_dentry->d_name.name,dst_dentry->d_name.len) !=NULL)
		return -EEXIST;
	if(dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
    	return -ENAMETOOLONG;

	ospfs_direntry_t *od = create_blank_direntry(d);
	if(IS_ERR(od))
		return PTR_ERR(od);
	//good to here
	...
	ospfs_inode_t *snode;
	snode = ospfs_inode(src_dentry->d_inode->i_ino);
	snode->oi_nlink++;
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
    ...
    ospfs_inode_t *node = NULL;//add this one
    while(entry_ino < ospfs_super->os_ninodes)
    {
    	node = ospfs_inode(entry_ino);
    	if( node && node->oi_nlink == 0)
    	{
    		node->oi_nlink+=1;
    		break;
    	}
    ...
    ospfs_direntry_t *n_dir = NULL;
    n_dir = create_blank_direntry(dir_oi);
    //check
    if(IS_ERR(n_dir))
    	return PTR_ERR(n_dir);
    ...
    n_dir->od_ino = entry_ino;
    memcpy(n_dir->od_name,dentry->d_name.name,dentry->d_name.len);
    n_dir->od_name[dentry->d_name.len] = 0;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	//no empty then return
	if(entry_ino >= ospfs_super->os_ninodes)
		return -ENOSPC;

	//open directory
	ospfs_direntry_t *n_dir = create_blank_direntry(dir_oi);
	if(IS_ERR(n_dir))
		return ERR_PTR(n_dir);
	//finish the node now
	node->oi_nlink = 1;
	node->oi_size = strlen(symname);
	node->oi_ftype = OSPFS_FTYPE_SYMLINK;
	strncpy(node->oi_symlink,symname,node->oi_size);
	node->oi_symlink[node->oi_size] = 0;

	//finish the directory
	n_dir->od_ino =entry_ino;
	strncpy(n_dir->od_name,dentry->d_name.name,dentry->d_name.len);
	n_dir->od_name[dentry->d_name.len]=0;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	{
		int colon_index = 0;//to record colon position

		// eprintk("root?\n");
//		if (colon_index == 0) 
//		{
			for (; colon_index < oi->oi_size + 1; colon_index++)
			{
...
//		}
		// eprintk("colon: %d\n", colon_index);
		// eprintk("colon: %c\n", oi->oi_symlink[colon_index]);
		// if (colon_index == oi->oi_size)
		// {
		// 	eprintk("link format error\n");
		// 	return (void *) 0;
		// }
