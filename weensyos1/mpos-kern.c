...
start(void)
{
	...
	for (i = 0; i < NPROCS; i++) 
	{
        ...
        for (j = 0; j < NPROCS; j++)
            proc_array[i].waited_pid[j] = 0;
        	//initialize value of waiting queue to 0
	}
...
static pid_t do_thread(void (*start_function)(void));
void
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
        ...
        for (j = 0; j < NPROCS; j++)
        {                  
            if (current->waited_pid[j] !=0)  //find process in wait queue
            {
                current->p_state = P_EMPTY; //clean up
                proc_array[j].p_state = P_RUNNABLE;// wake up processes who are waiting for current process to die
                proc_array[j].p_registers.reg_eax = current->p_exit_status;//set return value of wait()
                current->waited_pid[j] = 0;// set it to init status
            }
        }
		schedule();
	...
	case INT_SYS_WAIT: {
        ...
        {
            current->p_state = P_BLOCKED;	// set current process to BLOCKED
            proc_array[p].waited_pid[current->p_pid] = 1;  // process p is waited to die by current process
	...
	{
		void (*s_f)(void); 
		s_f = (void*)current->p_registers.reg_eax;
		current->p_registers.reg_eax = do_thread(s_f);
		run(current);
	}
	default:
		while (1)
...
static pid_t
do_thread(void (*start_function)(void))
{
	int i = 1;
    while(i < NPROCS)
    {
        ...
        {    
            special_registers_init(&proc_array[i]);
            proc_array[i].p_registers.reg_eip = (uint32_t)start_function;
            proc_array[i].p_registers.reg_esp = PROC1_STACK_ADDR + i * PROC_STACK_SIZE;
...
do_fork(process_t *parent)
{
	...
	for (i = 1; i < NPROCS; i++) 
	{
		if (proc_array[i].p_state == P_EMPTY)//find empty process
		{
			child = &proc_array[i];
			child->p_registers = parent->p_registers;//copy register to child processor
			child->p_registers.reg_eax = 0; //make sure child process return 0 to sys_fork()
			copy_stack (child, parent);//copy memory to child processor
	
			child->p_state = P_RUNNABLE;//set state to runnable
			return i;
		}
	}
	return -1;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// YOUR CODE HERE!
	pid_t sid = src->p_pid;
	pid_t did = dest->p_pid;

	size_t size;
	src_stack_top = PROC1_STACK_ADDR + sid*PROC_STACK_SIZE;
	src_stack_bottom = src->p_registers.reg_esp;
	size = src_stack_top - src_stack_bottom;//parent stack size
	dest_stack_top = PROC1_STACK_ADDR + did*PROC_STACK_SIZE; 
	dest_stack_bottom = 
		dest_stack_top + (src_stack_bottom - src_stack_top); 
