...
/******************************************************************************
 * MD5 AES RSA SUPPORTING FUNCTION
 */
...
//decrypt
		char cryname[FILENAMESIZ+11];
		strcpy (cryname, t->disk_filename);
		strcat (cryname, ".decrypt\0");
		const unsigned char key[] = "my key";
		decrypt(t->disk_filename, cryname, key);
		//Get MD5 on server
		assert(tracker_task->type == TASK_TRACKER);
...
static void task_upload(task_t *t)
{
	...
	char cryname[FILENAMESIZ+11];
	strcpy (cryname, t->filename);
	strcat (cryname, ".encrypt\0");
	const unsigned char key[] = "my key";
 
  	//srand(time(NULL));
 
  	aes_init();
  	encrypt(t->filename, cryname, key);
  	message ("encrypted\n");
	t->disk_fd = open(cryname, O_RDONLY);
	if (t->disk_fd == -1) {
...
{
	if( access( "privkey.pem", F_OK ) == -1 && access( "pubkey.pem", F_OK ) == -1) {
    // file doesn't exist
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	// Connect to the tracker and register our files.
		...
		register_files(tracker_task, myalias);
		//unregister_file(tracker_task, "cat1.jpg");
		//close(tracker_task->peer_fd);
		//message("attack %d\n", i++);
		}
	}
